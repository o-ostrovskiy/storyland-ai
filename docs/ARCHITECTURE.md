...
- **A per-run skip is only per-run if some other run succeeded (@el r4 FL-1, 2026-08-21).** The skip above discriminates on the exception *class*, and class cannot see whether a fault is run-scoped. A wrong or narrowed camelCase spelling in `_EXPERIMENT_ITEM_FIELDS` is a 400 on **every** request; so are an expired key, a revoked scope and a moved endpoint. Each arrives as a plain exception once per iteration, so every run was skipped and named, `run_lists` returned `[]`, and the pack was rebuilt from the legacy leg and reported as a success — *reaching exactly the state the bullet above argues the old behaviour was wrong for*, silently. `collect_experiment_candidates` now counts attempts and skips and raises `CalibrationDataError` when every attempted run was skipped; `attempted == 0` stays a legitimately empty leg, which is the state every dataset is in until PR2 moves the writes. ➡️ *The scope of a degrade is a claim about the fault's SUBJECT, and the subject is not readable from the type — it is readable from the outcome.*
- 🔴 **That rule was WITHDRAWN one round later, by its own author, and the correction is kept here rather than edited over (@el r5 ruling, 2026-08-22).** The sentence above is true; the predicate it asks for cannot express it. At `attempted == 1` there is no other run, so `skipped == attempted` is satisfied by **any** single failure and carries zero bits about systematicity — and `storyland_eval` with a monthly prefix routinely matches exactly one experiment, so n=1 is the middle of this distribution, not its tail. The counting rule therefore (a) turned an ordinary transient `list_items` failure into a build abort on the common case, and (b) **could not fire at n=1 on the 400 it was justified with**. ➡️ *An outcome-based discriminator inherits its population's SIZE as a hard bound on what it can distinguish* — the same error as citing a probe whose 248 items never took the branch, arriving from the other side.
- **So the fault is CLASSIFIED, primary, at every n.** An `ApiError` carrying a **4xx** raises `CalibrationDataError`: a bad `fields` spelling (400), an expired key (401), a revoked scope (403), a moved endpoint (404/405) are all the server stating that the *request* is wrong, and no retry and no sibling run makes that better. A **5xx**, a transport error, a timeout, or an `ApiError` with no status at all degrades to a per-run skip, regardless of count. ⚠️ **429 and 408 degrade despite being 4xx** — rate limiting and a request timeout are 4xx by number and transport-shaped by nature, and a monthly calibration that hits a rate limit must not abort the build. The classification is grounded against the pinned SDK, not against a double: langfuse raises *typed* subclasses (`UnauthorizedError`, `NotFoundError`, …) rather than bare `ApiError`s, so a row asserts that each one really does carry the fixed `status_code` the rule reads — a typed error that stopped setting it would degrade instead of raising, which is the failure direction that ships.
- **The count survives as a SECONDARY signal, gated at `attempted >= 2`.** It covers the one world classification misses: an endpoint that is genuinely down answers 5xx on every request, every run degrades correctly, and after 2026-11-16 the legacy leg is empty by construction — so the pack would go out thin and green. At `attempted >= 2` all-skipped is weak evidence, and weak evidence should still push a calibration pack fail-closed; at `attempted == 1` it is not evidence and must not fire.
- ⚠️ **Named residual, deliberately NOT fixed in PR1 (@el r5).** After this change a single transport failure at `attempted == 1` *does* yield a silently empty experiments leg. That is harmless today — the legacy leg is alive and carries everything — and it stops being harmless at **PR2**, when the writes move and the experiments leg becomes the only source. It is recorded on [MYS-909](https://linear.app/mystoryland/issue/MYS-909) as a required PR2 item, beside the run-level pagination item already parked there: *once the experiments leg is the sole source, a degraded leg must be distinguishable from an empty one at the caller, not just in the log.*
- **`root_span.update(metadata=…)` does NOT erase the `eval_id` written at span creation (Codex `r3834098109`, refuted 2026-08-22).** Codex read `run_scheduled_eval.py`'s four metadata updates and argued that Langfuse v4 keeps observation metadata in one OTel attribute, so each update replaces the mapping and drops this PR's `eval_id` — which would abort every calibration build that read such a run. Checked against the pinned SDK rather than reasoned about: `_flatten_and_serialize_metadata` emits one attribute **per key** (`langfuse.observation.metadata.<key>`) for a dict, and `set_attributes` merges by key, so an update naming `current_phase` cannot touch `eval_id`. ⚠️ The probe recorded above (54 runs / 248 items / 0 missing `eval_id`) could **not** have caught this either way: all three sibling writers pass only `input`/`output` to `root_span.update`, never `metadata`, so no run in that population exercised the path — `run_scheduled_eval.py` is the first writer that does. The claim therefore rests on the SDK's flattening and nothing else, so `TestSymbolPresence` now asserts it: a langfuse bump that serialises the dict whole reds there instead of dropping `eval_id` silently on the one writer that matters.

- **🔴 The systematic/transient classifier lives at the LEG'S CHOKE POINT, not only in the per-run loop (r6 FL-1, Codex `r3836481943`, 2026-08-22).** The r5 rule — a 4xx `ApiError` is evidence about the *request* and aborts, anything else degrades, with 429/408 carved out — went in at `list_items` inside the per-run loop. But the faults its own rationale names (expired key 401, revoked scope 403, endpoint moved 404/405) never reach `list_items`: they arrive at the leg's **first** call, `experiments.list`, or at `resolve_dataset_id`'s `datasets.get` before it. Both are outside the loop, so an `ApiError` there fell to `collect_candidates`'s broad `except` and emptied the whole leg behind a warning — and once the legacy leg retires on Nov 16 that is a green build with an empty pack, i.e. this ADR's own failure one call site above its fix. The classification now runs in that broad arm, which is the single funnel for `resolve_dataset_id`, `experiments.list`, the per-run loop and any call added to the leg later; the in-loop check stays, because it does a different job (scoping a degrade to one run and feeding the `attempted >= 2` count rule). ➡️ *A rule installed at the site where a fault was OBSERVED does not cover the site where it ENTERS* — so the boundary the rule is stated over is the leg, not the loop.

- **🔴 The count's population is the CLASSIFIER'S OWN LEFTOVERS, not every skip (r7 FL-1, Codex `r3837185006`, 2026-08-23).** The two rules above collided at the exact case one of them was written for. `_experiment_fault_is_systematic` carves out 429/408 and returns *degrade*; the `attempted >= 2 and skipped == attempted` guard then counted those same skips and raised — so a dataset with two matching experiments, every `list_items` call rate-limited, aborted the build. And rate limiting is precisely the fault that does **not** politely hit one request in three: it persists across consecutive requests, which is why 429 is in the carve-out at all. ➡️ *Two rules over the same population, one classifying and one counting, do not compose — the counter re-decides what the classifier already decided, without the evidence the classifier used.* The count now ranges over `countable_skips`: skips whose cause the classifier did **not** positively name retry-able (5xx, no status, non-`ApiError`), which is exactly the world it was kept for. One classification feeding one count over its own result. Two consequences worth stating: a **mixed** window (one 429, one 503) does not abort, because the guard requires every attempted run to have failed for a countable reason; and the carve-out is deliberately **narrow** — writing it as `not _experiment_fault_is_systematic(e)` would empty the counter entirely while reading like a narrowing, so a row refuses that shape.
...
- `tests/unit/test_experiment_run.py` — key/SDK pinning, the identity properties (stability, dataset separation, join injectivity), the read-side round trip, and two class guards: no file in the tree calls the retired endpoint, and every call site names its dataset.
