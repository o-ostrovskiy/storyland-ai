...
- **Bounded lock registry (r4).** `_session_locks` is an `OrderedDict`, not an unbounded `defaultdict`. `_get_session_lock` moves an entry to MRU on access and evicts least-recently-used, currently-unheld entries once the registry exceeds `_SESSION_LOCK_REGISTRY_CAP` — never evicting a lock that's held or has a pending waiter, so a genuinely-overlapping second caller for the same `job_id` can never be handed a different `Lock` instance.
